...
const forgotPasswordOtp = async (req, res) => {
  try {
    ...
    // ❌ Disable old OTPs
    await OTP.updateMany({ email }, { isActive: false });
    ...
    let otpReceiverEmail = email;

    // 🔐 If ADMIN → send to fixed admin email
    if (user.role === "Admin") {
      otpReceiverEmail = "darsiforyou@gmail.com";
    }

    await sendEmail({
      to: otpReceiverEmail,
      subject: "Darsi Password Reset OTP",
      html: `
        <h3>Darsi Security</h3>
        <p>Your OTP is:</p>
        <h2>${otp}</h2>
